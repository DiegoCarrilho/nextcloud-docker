...
ARQUIVOS=(
  "$BASE/config/nginx/site-confs/default|upstream php-handler {
    server 127.0.0.1:9000;
}
server {
    # listen on unencrypted HTTP since we're backproxying with Traefik
    listen 80;
    server_name _;
...
# Criar diretórios e arquivos com o conteúdo
for ITEM in "${ARQUIVOS[@]}"; do
  # Extrair o caminho do arquivo e o conteúdo
  ...
  echo "$CONTEUDO" > "$CAMINHO_ARQUIVO"
  echo "Arquivo criado: $CAMINHO_ARQUIVO"
done
